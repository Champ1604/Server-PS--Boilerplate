...
exports.commands = {
	fuse: function(target, room, user) {
		var pokemon = target.toLowerCase()
		target = target.toLowerCase()
		var dex = require("../data/pokedex.js").BattlePokedex
		var req = require("request")
		var cher = require("cheerio")
		var pokemon1 = target.split("/")[0]
        var pokemon2 = target.split("/")[1]
        if (dex[pokemon1] !== undefined) var pokemondex1 = dex[pokemon1].num
        else {
      		var pokemondex1 = "missingno"
        }
        if (dex[pokemon2] !== undefined) var pokemondex2 = dex[pokemon2].num
        else {
      		var pokemondex2 = "missingno"
        }
        var yeah = pokemon.replace(pokemon1, String(pokemondex1)).replace(pokemon2, String(pokemondex2));
        req("http://pokemon.alexonsager.net/" + yeah, function(err, resp, body){
            var $ = cher.load(body);
            var links = $("#pk_name");
            this.sendReplyBox("<strong>Fusion</strong>: " + links.text().replace(/\n/g, "").replace("␣", "") + " <strong>Link</strong>: " +"<a href=\"" + "http://pokemon.alexonsager.net/" + yeah + "\">" + links.text().replace(/\n/g, "").replace("␣", "") + "</a>")
        }.bind(this))
	},
	ip: 'whois',
	rooms: 'whois',
	...
	isearch: 'itemsearch',
	testing: function(target, room, user) {
		this.add("This command was a test, no need to be used most of the time.")	
	},
